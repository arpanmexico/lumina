...
class SellController
{
  ...
  public function getProductsByType($type){
    $database = new Database();
    $query = "SELECT id_armazon, modelo, (SELECT nombre FROM categorias WHERE armazones.id_marca = categorias.id_categoria) AS marca, precio, existencias, (SELECT nombre FROM categorias WHERE armazones.id_tipo = categorias.id_categoria) AS tipo FROM armazones WHERE id_tipo = '".$type."' AND suspendido = 0 AND existencias != 0 ORDER BY marca";
    $runQuery = $database -> query($query);
    $response = '';
    ...
    if($runQuery -> num_rows > 0){
      ...
      while($row = $runQuery -> fetch_array()){
        $response .= '
          <tr class="p-4 product-row" id="row'.$row['id_armazon'].'" onClick="selectProduct('; $response .= "'".$row['id_armazon']."', '".$row['modelo']."', '".$row['marca']."', ".$row['precio'].", ".$row['existencias'].", '".$row['tipo']."',1"; $response .= ')">
            <td class="">'.$row['modelo'].'</td>
            <td class="">'.$row['marca'].'</td>
            <td class=""> $ '.$row['precio'].' MXN</td>
  ...
  public function showSoldProducts($products){
    ...
    $query = "SELECT id_armazon, modelo, (SELECT nombre FROM categorias WHERE armazones.id_marca = categorias.id_categoria) AS marca, precio,(SELECT nombre FROM categorias WHERE armazones.id_tipo = categorias.id_categoria) AS tipo FROM armazones WHERE id_armazon IN (".$newProd.") ORDER BY marca";
